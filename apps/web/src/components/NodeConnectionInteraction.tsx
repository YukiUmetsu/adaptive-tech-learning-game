...
export default function NodeConnectionInteraction({
  ...
  const boundaryOffset = (nodeId: string, ux: number, uy: number) => {
    ...
    const radiusX = nodeSize.width / 2 + EDGE_PADDING_PX;
    const radiusY = nodeSize.height / 2 + EDGE_PADDING_PX;
  ...
  const edgeGeometry = (from: string, to: string) => {
    ...
    const startOffset = Math.min(boundaryOffset(from, ux, uy), distance / 2);
    const endOffset = Math.min(boundaryOffset(to, ux, uy), distance / 2);
