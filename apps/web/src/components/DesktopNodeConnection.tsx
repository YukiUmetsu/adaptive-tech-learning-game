...
import {
  nodeLabelMap,
  toggleEdge as toggleEdgeAnswer,
} from "../lib/nodeConnection";
import { stripInlineCode } from "../lib/inlineCode";
import InlineText from "./InlineText";

export interface DesktopNodeConnectionProps {
  nodes: GraphNode[];
  value: string[][];
  disabled?: boolean;
...
/**
 * Wide-screen `node_connection` renderer: positioned node pills with SVG
 * connection lines and drag-or-tap connection building.
 *
 * This is the original graph interaction; the mobile step-by-step builder in
 * `MobileNodeConnection` shares the same answer state and edge helpers.
 */
export default function DesktopNodeConnection({
  nodes,
  value,
  disabled = false,
  onChange,
}: DesktopNodeConnectionProps) {
  ...
  const toggleEdge = (from: string, to: string) =>
    onChange(toggleEdgeAnswer(value, from, to));
  ...
  return (
    <div className="connection connection-desktop">
      <p className="muted">
            ...
            <li key={`${from}->${to}`}>
              <span>
                <span className="edge-chip">
                  <InlineText text={nodeById.get(from) ?? from} />
                </span>
                <span aria-hidden="true" className="edge-arrow">
                  →
                </span>
                <span className="edge-chip">
                  <InlineText text={nodeById.get(to) ?? to} />
                </span>
              </span>
              <button
                type="button"
                ...
                aria-label={`Remove connection ${stripInlineCode(
                  nodeById.get(from) ?? from,
                )} to ${stripInlineCode(nodeById.get(to) ?? to)}`}
